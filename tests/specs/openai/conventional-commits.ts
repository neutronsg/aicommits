...
export default testSuite(({ describe }) => {
	...
	describe('Conventional Commits', async ({ test }) => {
		...
		async function runGenerateCommitMessage(gitDiff: string,
			...
			const commitMessages = await generateCommitMessage(OPENAI_KEY!, 'gpt-3.5-turbo', config.locale, 'test-branch', gitDiff, config.generate, config['max-length'], config.type, 7000);
